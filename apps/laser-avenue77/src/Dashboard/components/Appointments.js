...
await axios(requestBuilder( "appointments", "/appointments","get",
      ...
      for (let i = 0; i < appointmentData.length; i++) {
       ...
       dispatch(AdditionalAppointmentDataHandler(sortedData.slice(5,sortedData.length)))
        }
      });
    ...
    return (
     <View>
     <View>
     <Box  shadow={9} style={styles.styleAppoint} w="90%" rounded="xl" _text={{
     fontSize: "md",
     fontWeight: "medium",}}>
    <View>
    <Image
    ...
    <Center fontSize="xl"   >
    <Text style={{fontSize:14}}>  TODAY APPOINTMENTS
    </Text>    
    </Center>
    <Center    pb="20">
  ...
  <HStack style={{borderLeftWidth:8,borderColor:AddPriorityColor(item.Priorty), borderRadius:10,backgroundColor:'white',shadowColor: "#000",
shadowOffset: {
	width: 0,
	height: 12,
},
...
<Spacer />
<Text style={{fontSize:10 ,position:'absolute',right:0,top:10}}  >
...
const styles = StyleSheet.create({
  image1:{
    position:'absolute',top:-40,left:20,width:100,height:100,resizeMode: 'cover',shadowColor: "#000",
    shadowOffset: {
    width: 0,
    height: 12,
    },
  ...
  section22:{
    marginBottom:4,shadowColor: "#000",
   shadowOffset: {
   width: 0,
   height: 12,
   },
